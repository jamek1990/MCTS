...
void NMCS(int level){
	...
	ocena[t]=0;tabu[t]=false;
	}
	while(LevelS[0]!=0){
		cout<<"Liczba Gier: "<<LG<<"\n";
		LG=0;
		int temp_ile=ile;
		for(int i=0;i<LevelS[0];i+=(rand()%2+1)){
			dodaj(Level[0][i]);ile++;
			LevelS[1]=0;
			...
			//	cout<<"tabu\n";
				...
				int temp_ile_1=ile;
				//if(temp_ile+20<temp_ile_1)break;
				for(int j=0;j<LevelS[1];j+=(rand()%2+1)){
					for(int bt=0;bt<tabuCoSize;bt++)
						tabu[tabuCo[bt]]=false;
					tabuCoSize=0;
					int wiel = rand()%5;
					if(bestSolutionSize>100){
						for(int bt=0;bt<wiel;bt++){
							int tab = rand()%bestSolutionSize;
							tabu[bestSolution[tab]]=true;
							tabuCo[bt]=bestSolution[tab];
						}
						tabuCoSize=wiel;
					}
					dodaj(Level[1][j]);ile++;
					...
					while(LevelS[2]!=0){
						int temp_ile_2=ile;
						//if(temp_ile_1+10<temp_ile_2)break;
						for(int n=0;n<LevelS[2];n+=(rand()%2+1)){
							dodaj(Level[2][n]);ile++;
							LG++;
							layer_t_size=0;
							while(available_moves_size!=0){
								for(int t=0;t<available_moves_size;t++){
									...
									if(!tabu[hm]&&(ocena[hm]>=bestSeqLevelS[0] || ocena[hm]==0)){
										if(rand()%100<80)
										layer[t]=true;
										else
										layer[t]=false;
