...
int ocena2[6400][6400];
int rek[6400];
int maxl=400;
void random_layer_price_game2(int iq){
	//ruchy jako kolejka:)
	move move_exe;
	int ile=iq;
	int max_ile=0;
	...
	int qtime=0;
	int qt=1000;
	for(int i=0;i<6400;i++)ocena[i]=times[i]=0;
	move start[1000];
	...
	while(1){
		qtime++;
		if(max_ile2==max_ile)ok=1;
		if(qtime>doo){
			if(ok)PER-=3;
			cout<<"M:"<<PER<<" "<<max_ile2<<"\n";
			for(int i=0;i<6400;i++){
			if(rand()%100<PER)
			ocena[i]=rek[i]*(max_ile*PER/100);
			else
			ocena[i]=0;
			times[i]=0;
			}
		...
		while(available_moves_size!=0){	   
			for(int i=0;i<available_moves_size;i++){
				...
				if(ocena[hm]>=max_ile2 || ocena[hm]==0 || times[hm]<qtime){
					if(rand()%100<90)
					layer[i]=true;
					else
					layer[i]=false;
				...
				if(layer[i]){
					if(check_move(available_moves[i])){
						dodaj(available_moves[i]);
						ile++;
					}
				}else{
					layer_t[layer_t_size]=i;
					layer_t_size++;
				}
			}
			available_moves_size=0;
			for(int i=0;i<layer_t_size;i++)
				if(check_move(available_moves[layer_t[i]]))
					available_moves[available_moves_size++]=available_moves[layer_t[i]];
			layer_t_size=0;
		}
		...
		for(int i=0;i<ile;i++){
			...
			times[hm]=qtime+qt+(rand()%qt);//*(0.5*(max_ile2-ocena[hm]));
			
			//cout<<(1.0-0.5*(max_ile2-ocena[hm]))<<"\n";
		}
		if(max_ile<ile){
			PER=100;
			cout<<"score: "<<max_ile<<" "<<ile<<"\n";
			max_ile=ile;
			for(int i=0;i<6400;i++)rek[i]=0;
			for(int i=0;i<ile;i++){
				hm=hash_move(save_moves[i]);
				rek[hm]=1;
			}
